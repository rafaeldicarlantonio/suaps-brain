...
from ingest.pipeline import normalize_text  # keep import; useful later
...
def _retrieve(sb, index, query: str, top_k_per_type: int = 10) -> List[Dict[str, Any]]:
    """Lightweight retrieval for /chat (semantic-only; you already have hybrid rank in /search)."""
    vec = _embed(query)
    ...
    for ns in namespaces:
        ...
        for m in qr.matches or []:
            # prefer vector id "mem_<uuid>", otherwise metadata.id if you added it
            mid = (m.metadata or {}).get("id") or (m.id or "").replace("mem_", "")
            hits.append({"memory_id": mid, "namespace": ns, "score": float(m.score or 0)})

    # join with memories for titles/timestamps
...
def _pack_context(sb, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Fetch text for top items and return compact context [{"id","title","text"}...]"""
    if not items:
        return []
...
def _safe_json_answer(prompt: str, context: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Call the Answerer with a strict-JSON prompt.
    If parsing fails, retry once with a terse “valid JSON only” reminder.
    """
    sys = _load_prompt("prompts/system_answerer.md", "Return strict JSON.")
    ctx = [{"id": c["id"], "title": c["title"], "text": (c["text"][:2000])} for c in context[:8]]
    user = json.dumps({"question": prompt, "context": ctx})
...
def chat_chat_post(body: ChatReq, x_api_key: Optional[str] = Header(None)):
    ...
    # 1) session (robust: generate UUID here; don't rely on created_at to fetch)
    ...
    if not session_id:
        ...
        try:
            ...
            # best-effort: if insert failed but server generated one, try to read last inserted row
            try:
    ...
    if action == "revise":
        ...
        try:
            ...
            action = "block"  # fail safe if parse fails
    ...
    if action == "block":
        ...
        safe = {
            ...
            "redteam": verdict,
            "metrics": {"latency_ms": int((datetime.datetime.utcnow() - t0.replace(tzinfo=None)).total_seconds() * 1000)},
        }
        return safe
    ...
    # 7) persist messages (best-effort)
    try:
        ...
        sb.table("messages").insert(
            {
                ...
                "tokens": None,
                "latency_ms": int((datetime.datetime.utcnow() - t0.replace(tzinfo=None)).total_seconds() * 1000),
                "model": os.getenv("CHAT_MODEL"),
            }
        ).execute()
    except Exception:
